...
import { claseEvento, nombreAnimal, tipoDe, type Evento } from "./tipos";
...
export function CuadriculaMes({ dias, mes, corte, eventos }: { dias: string[]; mes: string; corte: string; eventos: Evento[] }) {
            ...
            >
              ...
              <ul className="space-y-1">
                {lista.slice(0, MAX_CHIPS).map((e, j) => (
                  <li key={j}>
                    <Link
                      href={e.animal_id ? `/animales/${e.animal_id}` : `#dia-${dia}`}
                      title={`${tipoDe(e.tipo).etiqueta} · ${nombreAnimal(e)}${e.detalle ? ` · ${e.detalle}` : ""}`}
                    ...
                    >
                      {tipoDe(e.tipo).etiqueta} · {e.chapeta || e.nombre || ""}
                    </Link>
                  </li>
                ))}
...
export function AgendaMes({ mes, corte, eventos }: { mes: string; corte: string; eventos: Evento[] }) {
        ...
        return (
          <li key={dia} id={`dia-${dia}`} className="scroll-mt-24 break-inside-avoid">
            ...
            <ul className="divide-y divide-tinta/5">
              {lista.map((e, j) => {
                const vencido = !e.realizado && e.fecha < corte;
                return (
                  <li key={j} className="flex flex-wrap items-center gap-x-3 gap-y-1 py-1.5 text-sm">
                    <span className={clsx("inline-block w-32 shrink-0 rounded-md border px-2 py-0.5 text-center text-xs font-bold", claseEvento(e, corte))}>
                      {tipoDe(e.tipo).etiqueta}
                    </span>
                    {e.animal_id ? (
                    ...
                    <span className="ml-auto">
                      {e.realizado ? <Etiqueta tono="verde">Hecho</Etiqueta> : vencido ? <Etiqueta tono="rojo">Vencido</Etiqueta> : <Etiqueta tono="azul">Programado</Etiqueta>}
                    </span>
                  </li>
                );
              })}
